...
impl Inode {
    ...
    /// Create a new directory entry using same inode
    pub fn link_at(&self, old_path: &str, new_path: &str) -> Option<Arc<Inode>> {
        ...
        let old_inode = self.read_disk_inode(op);
        let Some(old_inode_id) = old_inode else {
            return None;
        };
        ...
        get_block_cache(old_inode_block_id as usize, Arc::clone(&self.block_device))
            ...
            .modify(old_inode_block_offset, |old_inode: &mut DiskInode| {
                old_inode.ref_count += 1;
            });
        self.modify_disk_inode(|root_inode| {
            let file_count = (root_inode.size as usize) / DIRENT_SZ;
            let new_size = (file_count + 1) * DIRENT_SZ;
            self.increase_size(new_size as u32, root_inode, &mut fs);
            let dirent: DirEntry = DirEntry::new(new_path, old_inode_id);
            root_inode.write_at(
                file_count * DIRENT_SZ,
                dirent.as_bytes(),
                &self.block_device,
    ...
    pub fn unlink(&self, path: &str) -> bool {
        ...
        let Some(inode_id) = inode else {
            return false;
        };

        let cleaned = self.modify_disk_inode(|root_inode| {
            let file_count = (root_inode.size as usize) / DIRENT_SZ;
            let new_size = (file_count - 1) * DIRENT_SZ;
            let mut dirent = DirEntry::empty();
        ...
        let (block_id, block_offset) = fs.get_disk_inode_pos(inode_id);
        let mut clear_data_block = |disk_inode: &mut DiskInode| {
        ...
        get_block_cache(block_id as usize, Arc::clone(&self.block_device))
            ...
            .modify(block_offset, |disk_inode: &mut DiskInode| {
                ...
                // clear the data block if no reference
                if disk_inode.ref_count == 0 {
                    clear_data_block(disk_inode);
                }
            });
    ...
    pub fn stat(&self) -> Stat {
        let mut fs = self.fs.lock();
        let (inode_block_id, inode_block_offset) = fs.get_disk_inode_pos(self.inode_id);
        let (is_dir, ref_count) =
            get_block_cache(inode_block_id as usize, Arc::clone(&self.block_device))
                .lock()
                .read(inode_block_offset, |inode: &DiskInode| {
                    (inode.is_dir(), inode.ref_count)
                });
